...
@section("scripts")
    <script>
        function test() {
            ...
            if(tcnt1<1 && tcnt3<1) {
                $("form[name=season_term_form]").attr("action", '/info/season_term/{{isset($id)?$id:""}}');
                return true;
            }else {
                return false;
            }
        ...
        var ord = {{ sizeof($season_term)>0?sizeof($season_term):0 }};
        $(function () {
...
@section("contents")
    ...
    @include("admin.pc.include.price.season_list_2",['curPath'=>$curPath, 'curPath_staff'=>$curPathstaff, 'season'=>$season, 'id'=>isset($id)?$id:""])

    @include("admin.pc.include.price.season_price",['curPath'=>$curPath, 'curPath_staff'=>$curPathstaff, 'room'=>$room, 'season'=>$season, 'roomSeasonPrice'=>isset($roomSeasonPrice)?$roomSeasonPrice:[]])
@endsection
